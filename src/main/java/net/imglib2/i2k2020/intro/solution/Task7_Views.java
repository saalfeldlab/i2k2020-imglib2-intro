...
public class Task7_Views {
	...
	public static <T extends NumericType<T>> void outOfBounds(final RandomAccessibleInterval<T> img) {
		...
		ImageJFunctions.show(Views.interval(Views.extendZero(img), expansion)).setTitle("Zero outofbounds");
		ImageJFunctions.show(Views.interval(Views.extendMirrorSingle(img), expansion)).setTitle("Mirror outofbounds");
		ImageJFunctions.show(Views.interval(Views.extendPeriodic(img), expansion)).setTitle("Periodic outofbounds");

		// value outofbounds dorequires a value, we use the value of the first
		// pixel
		final T value = Views.iterable(img).firstElement().copy();

		ImageJFunctions.show(
				Views.interval(
						Views.extendValue(img, value),
						expansion))
				.setTitle("Value outofbounds");
	...
	public static <T extends RealType<T>> void outOfBoundsRealType(final RandomAccessibleInterval<T> img) {
		...
		ImageJFunctions.show(Views.interval(Views.extendRandom(img, 0, 255), expansion)).setTitle("Random outofbounds");
	}
